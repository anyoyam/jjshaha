...
尚未知道通过“模式”性测试的模式通常被称为原型模式。原型模式可能是某人建立的特殊的解决方案的工作结果并分享到社区里，但是也许因为它太年轻还没有机会经过严格审查。

此外，个人分享的模式可能没有时间或者兴趣通过“模式”性步骤，同时可能发布一个简短的关于它们原型模式的描述来代替。简单的描述或者小片段的这种模式一般被称为 plateles（？？）

全面记录一个合格模式所涉及的工作量非常巨大。回看一些设计模式最早期的工作，一种模式被认为是“好”的需要遵循以下几点：

- 解决特定问题：模式不能想象着去获取原理或者策略，它们需要获取解决方案。这是一个好的模式最最本质的部分。  
- 问题的解决方案不能太明显：我们发现解决问题的技术经常试着从众所周知的第一原则中得来。最好的设计模式往往间接地提供问题的解决方式 - 这被认识是那些具有挑战性的与设计有关的问题的必要方法。  
- 描述的观点必须已被证明：设计模式要求证明它们描述的功能，没有这些证明将必定不会被认真对待。如果一个模式的本质被高度质疑，也许只有勇敢的人才会尝试使用它。  
- 必须描述的是一种关系：在一些情况下模式描述的是一类模块。尽管一种实现方法会用已这种方式出现，但模式官方的描述必须说明更深的系统结构与机理来解释与代码的关系。  

我们应该用原谅的心情去思考那些不符合准则的原型模式就真的没有学习价值，当然，这是不切实际的。很多原型模式还是挺好的。我并不是说所有的原型模式看起来都有价值，但有一些很有用的会在未来的项目中帮助到我们。请在脑海中审判上面列出来的内容，你会在你选择的工程中比较容易些。

一个给合格模式的附加条件，它们显现出一些反复出现的现象。往往至少能胜任三个关键点，被称为三大规则。为了展现使用规则的重复，必须证明：

- 目的的适用性：模式是如何被认为是成功的？  
- 可用性：模式为什么会被认为是成功的？  
- 适用范围：—— 设计是否值得成为一种设计模式，因为它有更广的适用范围？如果是这样，这点就需要被证明。  

当评述或定义一种模式时，将上面几点记在脑海中是很重要的。

# 设计模式的结构

你也许好奇模式的作者是如何着手处理结构大纲，实现并考虑新模式的目的。一种模式最开始会展现出建立一下几点之间的关系的一种规则的格式：

- 上下文  
- 上下文中发生的作用力的系统  
- 一个配置文件，允许这些作用力在上下文中自己处理  

在脑海中记住这些，现在一起看下一种设计模式的组件元素的概括。一种设计模式必须包含有：

- **模式名称**和**描述信息**  
- **上下文概要** —— 上下文在那些情况下模式在给用户需要的响应中是有效的  
- **问题清单** —— 一份有指向的问题清单让我们更能理解模式的专注点  
- **解决方案** —— 描述用户的问题是如何在一份浅显易懂的概念和步骤清单中被解决的  
- **设计** —— 描述下模式的设计和特别点，用户使用它的交互行为  
- **实现方法** —— 模式如何被实现的手册  
- **图解** —— 模式中的类的可视化表达（如，Diagram）  
- **示例** —— 模式在最小格式下的实现方法  
- **相互依赖** —— 描述使用该模式可能需要的其他模式的支持  
- **关系** —— 那些模式是与本模式相似？是不是非常类似模仿其他？  
- **已知用法** —— 模式是不是在野外使用过？如果是，在哪里以及如何使用  
- **探讨** —— 团队或者作者认为的一些模式存在的好处  

设计模式是相当巨大的探讨，需要所有的团队或组织中的开发者在创建或维护解决方案时目标一致。
