var ctool = {
  ...
  throttle: function(fn, timeout) {
    ...
    return function(ctxt) {
      ...
      ctxt = ctxt || this;
      setTimeout(function() {
        fn && fn.apply(ctxt, [].slice.call(arguments, 1));
        canRun = true;
      }, timeout);
    }
  },
  ...
  workFlow: function(f, ctxt) {
    var fn = [],
      ...
      next = function() {
        var extr = extend.call($$, [].slice.call(arguments, 0)), n;
        ...
        if (!$$.once) {
          n = fn[fc];
          ((fn.length - 1) == fc) && ($$.status = 'finish', fc = 0, ft = '');
        } else {
          n = fn.shift();
          ft = fm.shift();
          (fn.length == 0) && ($$.status = 'finish', fc = 0, ft = '');
        }
        n && n.apply($$, [].slice.call(arguments, 0));
      },
      err = function(msg) {
        $$.status = 'finish';
        fc = 0;
    ...
    function meta() {
      return '[' + $$.varname + '@' + $$.filename + ':' + ($$.status == 'finish' ? 'finish' : ($$.once ? ft : fm[fc])) + ']';
    }
    return ($$ = {
      tool: {
        log: function(m) {
        ...
        goto: function(key) {
          if ($$.once === true) {
            console.warn && console.warn("This workflow can run only ONCE, please use command 'goto' carefully. the 'goto' will jump to is not you think. BE CAREFULL.");
          }
          var index = typeof key == 'number' ? key : fm.indexOf(key);
          if (index > fn.length - 1 || index < 0) {
            return next;
          }
          fc = --index;
      ...
      then: function(f, ctx, fname) {
        var _ = this;
        if (is(ctx, 'string') && typeof fname == 'undefined') {
          fname = ctx;
          ctx = false;
        }
        if (fname && fm.indexOf(fname) > -1) {
          throw new Error("The key of function '" + fname + "' has already existed.");
        }
        if (is(f)) {
          fn.push(function() {
            var a = arguments,
            ...
            try {
              $$.debug && console.log.call(console, meta(), arg.slice(2, arg.length));
              f.apply((ct || ctx || ctxt), arg);
            } catch (e) {
              _.status = 'finish';
              fc = 0;
              console && console.error(meta(), e);
              is(lv) && lv(e);
            }
          });
          if (!is(fname, 'string')) {
      ...
      run: function() {
        ...
        next.apply(this, arg);
        // console.log(meta(), 'fn[' + fn.length + ']', arg);
        // if (!this.once) {
        //   f1 = fn[fc];
        //   fn.length == 1 && (this.status = 'finish', fc = 0, ft = '');
        // } else {
        //   f1 = fn.shift();fm.shift();
        //   fn.length == 0 && (this.status = 'finish', fc = 0, ft = '');
        // }
        // typeof f1 == "function" && f1.apply(ctxt, arg);
      },
      tryrun: function() {
        if (this.status == "running") return;
        this.run.apply(this, [].slice.call(arguments, 0));
      },
      init: function(o) {
        return (o && (this.debug = !!o.debug, this.once = (o.once === false ? false : true), this.filename = o.filename || "undefined", this.varname = o.varname || "undefined")), this;
      },
      bind: function(o) {
        return (o && is(o, 'object') && (ctxt = o)), this;
      }
    }, (is(f) && $$.then(f)), $$);
  },
  funs: function(f, ctxt) {
    return this.workFlow(f, ctxt);
  },
  /***
      ## `.chain` 用来链式取值
      ### 用法：
